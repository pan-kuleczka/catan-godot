...
func get_node() -> Node:
	...
	var texture_path : String = "res://assets/cards/" + GameResource.e.keys()[resource] + ".svg"
	if FileAccess.file_exists(texture_path):
		return card_scene.instantiate().with_data(load(texture_path))
	push_warning("Card with resource " + str(resource) + " does not have a texture, falling back to a default one")
